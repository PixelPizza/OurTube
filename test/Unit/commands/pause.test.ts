...
import { PauseCommand } from "../../../src";

describe("PauseCommand tests", () => {
	describe("registerApplicationCommands tests", () => {
		test("Command registers a single chat input command", async () => {
			...
			const command = store.construct(PauseCommand, {
				name: "pause",
				path: "",
				root: "",
				extension: ".js"
			...
			expect(registry.chatInputCommands).toStrictEqual(new Set(["pause"]));
		});
	});
});
