...
import { RemoveCommand } from "../../../src";

describe("RemoveCommand tests", () => {
	describe("registerApplicationCommands tests", () => {
		test("Command registers a single chat input command", async () => {
			...
			const command = store.construct(RemoveCommand, {
				name: "remove",
				path: "",
				root: "",
				extension: ".js"
			...
			expect(registry.chatInputCommands).toStrictEqual(new Set(["remove"]));
		});
	});
});
