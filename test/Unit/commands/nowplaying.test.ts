...
import { NowPlayingCommand } from "../../../src";

describe("NowPlaying tests", () => {
	describe("registerApplicationCommands tests", () => {
		test("Command registers a single chat input command", async () => {
			...
			const command = store.construct(NowPlayingCommand, {
				name: "nowplaying",
				path: "",
				root: "",
				extension: ".js"
			...
			expect(registry.chatInputCommands).toStrictEqual(new Set(["nowplaying"]));
		});
	});
});
