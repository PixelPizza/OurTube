...
import { SkipCommand } from "../../../src";

describe("SkipCommand tests", () => {
	describe("registerApplicationCommands tests", () => {
		test("Command registers a single chat input command", async () => {
			...
			const command = store.construct(SkipCommand, {
				name: "skip",
				path: "",
				root: "",
				extension: ".js"
			...
			expect(registry.chatInputCommands).toStrictEqual(new Set(["skip"]));
		});
	});
});
